 ...
 * @throws DatanestResponseError Request HTTP server or validation error
 */
export async function getAllEnviroChemicals(
...
export async function getProjectMatrices(
  ...
  const response = await client.get(
    "v1/projects/" + projectUuid + "/enviro/matrices"
  );
  return (await response.json()) as {
    matrices: {
      matrix_id: number;
...
export async function getProjectScenarios(
  client: DatanestClient,
  projectUuid: string
): Promise<{ scenarios: ProjectScenario[] }> {
  const response = await client.get(
    "v1/projects/" + projectUuid + "/enviro/scenarios"
  );
  return await response.json();
}
 ...
 * @throws DatanestResponseError Request HTTP server or validation error
 */
export async function getProjectScenarioStandards(
  client: DatanestClient,
  projectUuid: string,
  scenarioId: number,
  filters?: {
  ...
  PaginatedResponse<GuidelineStandard> & {
 ...
 * @throws DatanestResponseError Request HTTP server or validation error
 */
export async function getProjectScenarioGuidelines(
  client: DatanestClient,
  projectUuid: string,
  scenarioId: number,
  filters?: {
 ...
 * @throws DatanestResponseError Request HTTP server or validation error
 */
export async function getProjectSampleChemicalResults(
  client: DatanestClient,
  projectUuid: string,
  filters?: {
    casno?: string[];
  ...
  const response = await client.get(
    "v1/projects/" + projectUuid + "/enviro/samples/chemical-results",
    filters
  );
  return await response.json();
}
 ...
 * @throws DatanestResponseError Request HTTP server or validation error
 */
export async function getProjectSampleLocations(
  client: DatanestClient,
  projectUuid: string,
  filters?: {
    bbox?: BBox;
  ...
  const response = await client.get(
    "v1/projects/" + projectUuid + "/enviro/samples/locations",
    filters
  );
  return await response.json();
}
...
export async function getProjectSamples(
  ...
  const response = await client.get(
    "v1/projects/" + projectUuid + "/enviro/samples",
    filters
  );
  return await response.json();
}
